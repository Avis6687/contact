...
contactRouter.delete('/contact/:id',async(req,res)=>{
    ...
    try{
        const deleteContactList= await pool.query('DELETE FROM clientinfo WHERE id=$1',[id])
        res.status(201).json({ success: true, data:{message:"Successfully deleted"} });
    }catch(err){
        res.status(400).json({success: false, data:{message: "Contact delete unsuccessfull"}})
    }
})
...
contactRouter.put('/contact/verify/:id',async(req,res) =>{
    ...
    try{
        const editContactList = await pool.query(`UPDATE clientinfo SET is_verified=$1 WHERE id =$2 RETURNING *`,[is_verified,id])
